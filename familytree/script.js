...
class Universe {
  ...
  loadFamilyTree(treeFile_) {
    let newFTree = new FamilyTree(this, treeFile_);

    this.familyTrees.push(newFTree);
    this.addTree(newFTree.tree);
  }
  ...
  addTree(tree) {
    this.treesDom.appendChild(tree.dom);
...
request.onload = function() {
  let treeFile = request.response;
  u_.loadFamilyTree(treeFile)
}
request.open('GET', "example1.txt");
